# =============================================================================
# ATAC-seq differential accessibility (csaw framework)
...
# Figures: Fig 7 (framework)
# Environment: ATAC env (R 4.4.1)   (exact versions in ../envs/ and README.md)
# Provenance: original project script "Analyzing_ATACseq_2.R" — analysis logic UNCHANGED; only this
...
cKO.peaks <- read.table("Tox_cKO_peaks.filt.broadPeak", sep="\t")[,1:3]
FF.peaks <- read.table("Tox_FF_peaks.filt.broadPeak", sep="\t")[,1:3]
colnames(cKO.peaks) <- c("chrom", "start", "end")
colnames(FF.peaks) <- c("chrom", "start", "end")
...
pe.bams <- c("Tox_FF.sorted.noDups.filt.noMT.bam", "Tox_cKO_sub.sorted.noDups.filt.noMT.bam")
...
CPM.df <- cbind(log2FC=log2((peak.counts.filt@assays@data$counts[,1] + 1)/(peak.counts.filt@assays@data$counts[,2] + 1)), avgLogCPM=aveLogCPM(asDGEList(peak.counts.filt)))

##############################

# get paired-end fragment size distribution
cKO.pe.sizes <- getPESizes("Tox_cKO_sub.sorted.noDups.filt.noMT.bam")
FF.pe.sizes <- getPESizes("Tox_FF.sorted.noDups.filt.noMT.bam")
gc()
# plot
hist(cKO.pe.sizes$sizes) # repeat for all replicates and conditions
hist(FF.pe.sizes$sizes) # repeat for all replicates and conditions
...
colnames(y$counts) <- c("FF", "cKO")
rownames(y$samples) <- c("FF", "cKO")
y$samples$group <- c("control", "ko")
design <- model.matrix(~0+group, data=y$samples)
colnames(design) <- c("control", "ko") # CONFIRM THAT THESE COLUMNS CORRECTLY ALIGN!!
# design
...
ggplot(data=data.frame(merged.peaks), aes(x = logCPM, y = logFC)) + 
  geom_point() + scale_color_manual(values = c("black", "red")) + 
  geom_smooth(inherit.aes=F, aes(x = logCPM, y = logFC), method = "loess") + # smoothed loess fit; can add span=0.5 to reduce computation load/time
  geom_hline(yintercept = 0) + labs(col = NULL)
